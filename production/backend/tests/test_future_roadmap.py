...
from app.services.live_driver_profile import detect_soc_rise_charging, live_driver_profile
...
def test_bad_data_vehicle_is_excluded_and_baseline_seed_is_used(db_session):
    fleet = Fleet(name="Evify", city="Surat")
    db_session.add(fleet)
    db_session.flush()
    driver = Driver(fleet_id=fleet.id, driver_code="D2", full_name="Driver 2")
    bad_vehicle = Vehicle(fleet_id=fleet.id, vehicle_code="GJ05PZ1856")
    db_session.add_all([driver, bad_vehicle])
    db_session.flush()
    db_session.add(
        Telemetry(
            vehicle_id=bad_vehicle.id,
            driver_id=driver.id,
    ...
    assert profile["profile_status"] == "baseline_seed"
    assert profile["baseline_seed"]["avg_speed_kmph"] == 12.4
    assert "GJ05PZ1856" in profile["excluded_vehicle_codes"]
...
def test_fleet_map_and_weekly_report_are_scoped_and_deterministic(db_session):
    class UserStub:
        role = "fleet_operator"
        fleet_id = None
        driver_id = None
