...
### 2.6.1 Context Signals And Inference Readiness

Codebase audit status for operational context signals:

| Signal | Current implementation status | Production note |
|---|---|---|
| Ignition on/off | Built foundation | `evify_adapter.py` parses `IgnitionOn`; telemetry, serializers, trip inference, wait classifier, live profile, and live decisions use `ignition_on`. |
| Traffic | Built foundation | `external_context.py` uses Google Directions with `duration_traffic_min`, `traffic_index`, `eta_delay_min`, timeout, cache, quota guard, fallback, and H3 cache buckets. Real accuracy depends on `GOOGLE_MAPS_API_KEY`. |
| Weather | Built foundation | `external_context.py` uses OpenWeather with ambient temp/rain/humidity/wind fields, timeout, cache, quota guard, fallback, and H3 cache buckets. Real accuracy depends on `OPENWEATHER_API_KEY`. |
| Elevation | Built foundation | `external_context.py` uses Google Elevation when configured and fallback grade/elevation otherwise. |
| Time bucket/day context | Partially built | `live_driver_profile.py` computes date, month, season, day of week, hour, slot, and weekend flag. `route_scorer.py` accepts `day_type` and `slot`. |
| Occasion/festival/holiday | Not implemented yet | Placeholder fields exist as `holiday_flag=false`, `festival_event_flag=false`, and `event_source=not_configured`; there is no calendar feed or operator event UI yet. |
| Area/H3 | Partially built | H3 is implemented for external API cache bucketing and cost reduction. Telemetry does not yet persist H3 cells, and H3/PostGIS zone-intelligence tables are not implemented. |
| Stop/wait reason | Built foundation | `wait_classifier.py` classifies `restaurant_wait`, `charging_wait`, `traffic_wait`, `idle_wait`, and `moving` using speed, ignition, charge plug, restaurant proximity, and charger proximity. |
| Opportunistic charging at stops | Partial | `live_driver_profile.py` can recommend using a stop window to top up when low SOC, stopped, ignition on, and a nearby charger exists; a full context-inference + notification policy is still pending. |

Target next backend shape:

```text
context signal collector
  -> context inference engine
  -> state + confidence + evidence
  -> charging/route/notification decision
  -> logged outcome for evals
```

The system should not claim to know pickup/drop-off/occasion context as fact unless provided by driver, fleet dispatch, or an official integration. Inferred context must stay confidence-labeled.
