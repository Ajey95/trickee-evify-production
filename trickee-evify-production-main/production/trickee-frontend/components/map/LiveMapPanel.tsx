...
function vehicleIconHtml(point: VehiclePoint) {
  ...
  const recordedTime = point.recorded_at ? new Date(point.recorded_at).getTime() : 0;
  const isStale = recordedTime ? (Date.now() - recordedTime) > 5 * 60 * 1000 : false;
  return `
    <div class="trickee-vehicle-marker ${riskTone(point.risk_level)} ${isStale ? 'stale' : ''}">
...
function vehicleIconHtmlCompact(point: VehiclePoint) {
  const soc = Number(point.soc).toFixed(0);
  const recordedTime = point.recorded_at ? new Date(point.recorded_at).getTime() : 0;
  const isStale = recordedTime ? (Date.now() - recordedTime) > 5 * 60 * 1000 : false;
  return `
...
function VehicleMarker({ point }: { point: VehiclePoint }) {
  ...
  const isStale = recordedTime ? (Date.now() - recordedTime) > 5 * 60 * 1000 : false;
  return (
    <div className={`trickee-vehicle-marker ${riskTone(point.risk_level)} ${isStale ? 'stale' : ''}`}>
  ...
  React.useEffect(() => {
    ...
    if (visible.vehicles) {
      for (const point of vehicles) {
        ...
        const lastSeenStr = formatLastSeen(point.recorded_at);
        const riskLevel = point.risk_level || "low";
        const riskScore = point.battery_risk_score ?? 0;
        const archDisplay = point.archetype_display || point.archetype_label || "";
        const titleText = `${point.driver_code} - ${Number(point.soc).toFixed(1)}% SOC\n` +
          (archDisplay ? `Archetype: ${archDisplay.replaceAll("_", " ")}\n` : "") +
          `Risk: ${riskLevel} (${Number(riskScore).toFixed(0)}) · Updated: ${lastSeenStr}`;

        const tooltipHtml = `
          <div class="space-y-1">
            <div class="flex items-center justify-between gap-4 font-semibold text-white">
              <span class="text-xs">${escapeHtml(point.driver_code)}</span>
              <span class="text-xs text-[#00b4d8] font-mono">${Number(point.soc).toFixed(0)}% SOC</span>
            </div>
            ${archDisplay ? `
              <div class="text-[10px] text-gray-400 flex items-center gap-1">
                <span>Archetype:</span>
                <span class="text-white capitalize">${escapeHtml(archDisplay.replaceAll("_", " "))}</span>
              </div>
            ` : ""}
            <div class="text-[10px] text-gray-400 flex items-center justify-between gap-3">
              <span>Risk: <span class="capitalize ${
                riskLevel === "high"
                  ? "text-[#df6d63] font-bold"
                  : riskLevel === "medium"
                  ? "text-[#c69b55]"
                  : "text-[#75c995]"
              }">${escapeHtml(riskLevel)} (${Number(riskScore).toFixed(0)})</span></span>
              <span class="text-[9px] text-gray-500 font-mono">${escapeHtml(lastSeenStr)}</span>
            </div>
          </div>
        `;
