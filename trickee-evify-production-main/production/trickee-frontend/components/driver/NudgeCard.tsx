...
export const NudgeCard = ({ nudge, onOutcomeUpdated }: NudgeCardProps) => {
  const [currentOutcome, setCurrentOutcome] = useState<string | null>(nudge?.outcome || null);
  const [isUpdating, setIsUpdating] = useState(false);

  useEffect(() => {
    setCurrentOutcome(nudge?.outcome || null);
  }, [nudge]);

  const handleOutcome = async (outcome: string) => {
    if (!nudge?.id) return;
    setIsUpdating(true);
    const res = await api.intelligence.updateNudgeOutcome(nudge.id, outcome);
    if (res.success) {
  ...
  return (
    <Card className="bg-accent-teal/5 border-accent-teal/30 relative overflow-hidden">
      ...
      {nudge.id && (
        <div className="mt-6 pt-6 border-t border-bg-border/40 flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-[10px] text-text-dim uppercase font-bold tracking-widest font-mono">Compliance Status</p>
            {currentOutcome ? (
              <span className="text-sm font-semibold capitalize text-accent-teal">
                Logged: {currentOutcome.replaceAll("_", " ")}
              </span>
            ) : (
              <span className="text-sm font-semibold text-text-dim font-mono">Awaiting Response</span>
            )}
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              variant={currentOutcome === "accepted" ? "primary" : "outline"}
              size="sm"
              isLoading={isUpdating}
              onClick={() => handleOutcome("accepted")}
              className={currentOutcome === "accepted" ? "bg-accent-teal text-bg-primary" : ""}
            >
              Accept Route
            </Button>
            <Button
              variant={currentOutcome === "heading_to_charger" ? "primary" : "outline"}
              size="sm"
              className={currentOutcome === "heading_to_charger" 
                ? "bg-accent-amber text-bg-primary border-accent-amber" 
                : "hover:bg-accent-amber/10 hover:text-accent-amber hover:border-accent-amber/20 text-accent-amber border-accent-amber/35 bg-accent-amber/5"}
              isLoading={isUpdating}
              onClick={() => handleOutcome("heading_to_charger")}
            >
              Heading to Charger
            </Button>
            <Button
              variant={currentOutcome === "dismissed" ? "primary" : "ghost"}
              size="sm"
              className={currentOutcome === "dismissed" 
                ? "bg-accent-red text-bg-primary hover:bg-accent-red/90" 
                : "hover:bg-accent-red/10 hover:text-accent-red text-text-dim"}
              isLoading={isUpdating}
