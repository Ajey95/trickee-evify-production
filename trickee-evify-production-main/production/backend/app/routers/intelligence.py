...
async def wait_time(
    ...
    record_id = None
    if payload.persist and payload.vehicle_id:
        record = WaitEvent(
            vehicle_id=payload.vehicle_id,
            driver_id=payload.driver_id,
            started_at=datetime.utcnow(),
            last_seen_at=datetime.utcnow(),
            wait_type=result.get("wait_type", "ready"),
            source="manual",
            ignition_on=payload.ignition_on or False,
            charge_plug=payload.charge_plug or False,
            lat=payload.driver_location.lat,
            lng=payload.driver_location.lng,
            duration_seconds=int(result.get("estimated_wait_min", 0) * 60),
            confidence=0.9,
        ...
        db.refresh(record)
        record_id = record.id
    return ok({**result, "decision_record_id": record_id})
...
class NudgeOutcomeUpdate(BaseModel):
    outcome: str
...
def update_nudge_outcome(
    nudge_id: str,
    payload: NudgeOutcomeUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    nudge.outcome = payload.outcome
    if payload.outcome in ["accepted", "heading_to_charger", "dismissed", "Accepted", "Rejected", "Ignored"]:
        nudge.acknowledged_at = datetime.utcnow()
        nudge.status = "acknowledged"
    else:
        nudge.status = "updated"
    nudge.updated_at = datetime.utcnow()
    db.commit()
    return ok(nudge_event_dict(nudge))


@router.post("/waits/{wait_id}/outcome")
def update_wait_outcome(
    wait_id: str,
    payload: DecisionOutcomeUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_roles("trickee_admin", "fleet_operator")),
):
    wait = db.get(WaitEvent, wait_id)
    if not wait:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Wait event not found")
    ctx = wait.context or {}
    ctx["outcome"] = payload.outcome
    ctx["operator_feedback"] = payload.feedback
    wait.context = ctx
    db.commit()
    return ok(wait_event_dict(wait))


@router.post("/order-assignments/{decision_id}/outcome")
def update_order_outcome(
    ...
    decision = db.get(OrderAssignmentDecision, decision_id)
    if not decision:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Decision not found")
    decision.outcome = payload.outcome
    res_payload = decision.result_payload or {}
    res_payload["operator_feedback"] = payload.feedback
    decision.result_payload = res_payload
    db.commit()
    return ok(order_assignment_decision_dict(decision))


@router.post("/charging-decisions/{decision_id}/outcome")
def update_charging_outcome(
    decision_id: str,
    payload: DecisionOutcomeUpdate,
    db: Session = Depends(get_db),
    current_user: User = Depends(require_roles("trickee_admin", "fleet_operator")),
):
    decision = db.get(ChargingDecisionRecord, decision_id)
    if not decision:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Decision not found")
    decision.outcome = payload.outcome
    res_payload = decision.result_payload or {}
    ...
    decision.result_payload = res_payload
    db.commit()
    return ok(charging_decision_record_dict(decision))
