...
export const DriverProfileCard = ({ driver, currentVehicle, archetype }: DriverProfileCardProps) => {
  return (
    <Card className="flex flex-col md:flex-row gap-8 items-center md:items-start border-bg-border/60">
      ...
      <div className="flex-1 text-center md:text-left">
          ...
          <div className="flex items-center gap-2 justify-center md:justify-start flex-wrap">
            ...
            {archetype && (
              <Badge variant="info" className="capitalize">
                {archetype.display_name || archetype.label?.replaceAll("_", " ")}
              </Badge>
            )}
          </div>
        </div>
        ...
        <div className={`grid grid-cols-2 ${archetype ? "md:grid-cols-3 lg:grid-cols-5" : "lg:grid-cols-4"} gap-6`}>
          <div>
          ...
          {archetype && (
            <div>
              <p className="kpi-label">Archetype Conf</p>
              <p className="text-xl font-bold font-mono text-accent-teal">
                {Math.round((archetype.confidence || 0) * 100)}%
              </p>
              <p className="text-[10px] text-text-dim mt-0.5">AI confidence</p>
            </div>
          )}
        </div>
      </div>
