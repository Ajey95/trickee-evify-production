...
def live_driver_decision(
    ...
    *,
    ...
    nudge_id_str = None
    outcome = None
    nudge_status = "created"
    
    # Look up the latest persisted nudge from the database
    latest_nudge = db.query(NudgeEvent).filter(NudgeEvent.driver_id == driver.id).order_by(NudgeEvent.created_at.desc()).first()

    if (persist_nudge or not latest_nudge) and latest and driver.id:
        ...
        db.add(nudge_event)
        db.commit()
        db.refresh(nudge_event)
        nudge_id_str = nudge_event.id
        outcome = nudge_event.outcome
        nudge_status = nudge_event.status
        latest_nudge = nudge_event
    elif latest_nudge:
        nudge_id_str = latest_nudge.id
        outcome = latest_nudge.outcome
        nudge_status = latest_nudge.status
        nudge_message = latest_nudge.message
        if latest_nudge.payload:
            charging = latest_nudge.payload.get("charging") or charging
            route = latest_nudge.payload.get("route") or route
            personalized_range = latest_nudge.payload.get("personalized_range") or personalized_range
            wait = latest_nudge.payload.get("wait") or wait

    return {
        "driver_id": driver.id,
        "driver_code": driver.driver_code,
        "generated_at": latest_nudge.created_at.isoformat() if latest_nudge else datetime.utcnow().isoformat(),
        "profile": profile,
        "personalized_range": personalized_range,
        "wait_classification": wait,
        "charging_recommendation": charging,
        "route_recommendation": route,
        "nudge_id": nudge_id_str,
        "outcome": outcome,
        "status": nudge_status,
        "driver_nudge": {
            "id": nudge_id_str,
            ...
            vehicle_points.append(
                {
                    ...
                    "soc": latest.soc,
                    "speed": latest.speed,
                    "risk_level": battery.get("risk_level", _battery_risk_level(latest)),
