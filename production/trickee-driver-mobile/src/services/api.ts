...
export const api = {
  ...
  signupOptions(signal?: AbortSignal) {
    return request<{vehicles: SignupVehicleOption[]}>('/auth/signup-options', {
      signal,
    });
  },
  ...
  unregisterFcmToken(
    ...
    return request<{registered: boolean}>('/auth/fcm-token', {
      method: 'DELETE',
      token,
      body: {token: fcmToken, platform, device_label: deviceLabel},
    });
  ...
  postLocation(token: string, payload: MobileLocationPayload) {
