...
import { Route, Clock, Zap, Map } from "lucide-react";
      ...
      {routes.map((route) => (
        <Card 
          ...
          className={cn(
            ...
            route.rank === 1 && "border-accent-teal/50 bg-accent-teal/[0.02]"
          )}
        >
          {route.rank === 1 && (
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className={cn(
              ...
              route.rank === 1 ? "bg-accent-teal/20 border-accent-teal/30 text-accent-teal" : "bg-bg-primary border-bg-border text-text-dim"
            )}>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end">
              <div>
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-xl font-bold font-mono text-text-primary">{route.personalized_eta_min.toFixed(1)}</span>
            ...
            <div className="h-1.5 bg-bg-border rounded-full overflow-hidden">
               <div 
                className={cn("h-full transition-all duration-1000", route.rank === 1 ? "bg-accent-teal" : "bg-bg-border")} 
                style={{ width: `${(1 / route.composite_score) * 50}%` }}
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-bg-border/40">
              <div>
                ...
                <p className="text-sm font-bold font-mono text-text-primary">{route.soc_end_pct}%</p>
              </div>
              <div className="text-right">
