...
export default function DriverProfilePage() {
  ...
  const mappedNudge = useMemo(() => {
    if (!liveDecision) return null;
    const route = liveDecision.route_recommendation || {};
    const nudge = liveDecision.driver_nudge || {};
    const currentSoc = liveDecision.profile?.latest?.soc ?? 45;
    const estimatedRange = liveDecision.personalized_range?.estimated_range_km ?? 38;
    const requiredSoc = route.soc_required_pct ?? 0;

    return {
      id: liveDecision.nudge_id || nudge.id || liveDecision.id,
      outcome: liveDecision.outcome || nudge.outcome,
      recommended_departure: route.departure_nudge?.recommended_departure || route.recommended_departure || "Immediate",
      buffer_applied_min: route.departure_nudge?.buffer_applied_min || route.buffer_applied_min || 0,
      message: nudge.message || "No active recommendation.",
      route_name: route.destination?.label || (route.destination ? `${route.destination.lat.toFixed(4)}, ${route.destination.lng.toFixed(4)}` : "Standard route"),
      soc_start: currentSoc,
      soc_end: Math.max(0, currentSoc - requiredSoc),
      range_remaining_km: estimatedRange,
  ...
  const refreshLiveDecision = async () => {
    if (!driver) return;
    const liveDecisionResult = await api.intelligence.driverLiveDecision(driver.id);
    if (liveDecisionResult.success) {
      setLiveDecision(liveDecisionResult.data);
    }
  };
        ...
        {driver && (
          <>
            <DriverProfileCard 
              ...
              archetype={liveProfile?.archetype || behavior?.archetype}
            />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-8">
                <section>
                  ...
                  <NudgeCard nudge={mappedNudge} onOutcomeUpdated={refreshLiveDecision} />
                </section>
                    ...
                    {liveProfile?.profile_status === "live" ? (
                      <>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="p-4 rounded-xl border border-bg-border bg-bg-primary/40 col-span-2 flex justify-between items-center">
                            <div>
                              <p className="text-xs font-bold text-text-dim uppercase tracking-wider mb-1">AI Archetype</p>
                              <p className="text-sm font-bold text-accent-teal capitalize">
                                {liveProfile.archetype?.display_name || liveProfile.archetype?.label?.replaceAll("_", " ") || "Learning..."}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="text-xs font-bold text-text-dim uppercase tracking-wider mb-1">Confidence</p>
                              <p className="text-sm font-bold text-text-primary font-mono">
                                {Math.round((liveProfile.archetype?.confidence || 0) * 100)}%
                              </p>
                            </div>
                          </div>
